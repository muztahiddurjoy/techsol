  ...
  return (
    <div className='relative  h-[100vh] w-full top-0 left-0 right-0 bottom-0'>
        ...
        <div className="absolute top-0 left-0 right-0 bottom-0 flex items-center px-5 md:px-10 z-30">
            <p className={`text-5xl uppercase font-bold transition-all ease-in-out duration-500 delay-300 ${anim?'ml-0 opacity-100':'-ml-10 opacity-0'} text-white`}>The Reliable Address <br /> <span className='text-primary'>for energy solutions</span></p>
        </div>
        
        <div className={`absolute top-0 left-0 bottom-0 right-0 bg-gradient-to-tl from-white to-transparent`}>
        <Image src="/solar.png" height={400} width={600} alt='' className={`z-40 absolute transition-all ease-in-out delay-300 duration-500 bottom-0 ${anim?'right-5 opacity-100':'-right-5 opacity-0'}`}/>
        </div>
    </div>
  )
}
